...
switch($tab){
	case 'add':
		$sortorder=dofetch(doquery("select count(id) from body_type",$dblink));
		$sortorder=$sortorder[0]+1;
		doquery( "insert into body_type(title, sortorder) values('', '".$sortorder."')", $dblink );
		$id = inserted_id();
		?>
			<tr data-id="<?php echo $id?>">
                ...
                <td class="text-center">
                    ...
                    <a href="body_type_manage.php?id=<?php echo $id;?>&tab=status&s=0" class="change_status_sortable">
                        <img src="images/offstatus.png" alt="Off" title="Set Status On">
                    </a>
                    <a onclick="" class="delete_record_sortable" href="body_type_manage.php?id=<?php echo $id;?>&amp;tab=delete"><img title="Delete Record" alt="Delete" src="images/delete.png"></a>
                </td>
            </tr>
		<?php
		die;
	...
	case 'edit':
		extract( $_POST );
		doquery( "update body_type set title = '".$title."' where id = '".$id."' ", $dblink );
		die;
	break;
	case 'delete':
	...
	case 'status':
		if(isset($_GET["id"]) && !empty($_GET["id"])){
			...
			echo 'body_type_manage.php?id='.slash($_GET["id"]).'&tab=status&s=';
			if( $_GET[ "s" ] == 0 ) {
	...
	break;
	case 'bulk_action':
		include("modules/body_type/bulkactions.php");
	break;
}
?>
	...
	<div class="container-widget row">
    	<div class="col-md-12">
		  <?php
            switch($tab){
                case 'list':
                    include("modules/body_type/list.php");
                break;
            }
          ?>
    	</div>
