...
doquery( "SET time_zone='+05:00';", $dblink );
/*--------------Site Configuration--------------*/
...
$condition = array("Good", "Very Good", "Excellent", "Running Condition", "Not Moveable");
$fuel_type = array("CNG", "Diesel", "Gasoline/Petrol", "LP Gas", "Hybrid");
$transmission_type = array("Automatic", "Manual");
$drive_hand = array("Left Hand", "Right Hand");
$drive_types = array("2WD", "4WD");
function admin_logo(){
	global $admin_logo, $site_title, $site_url;
	echo '<a href="'.$site_url.'/restaurant">';
	if(!empty($admin_logo)){
...
$product_type = array("Sale Item","Purchase Item", "Wheat Pisai");
$unit_type = array("Piece","Kg","Litre");
...
function datetime_convert($date_added, $with_sec = ""){
	return date("d/m/Y h:i".(!empty($with_sec)?":s":"")." A", strtotime($date_added));
}
...
function get_order( $id ) {
	global $dblink;
	$sale= dofetch(doquery("select * from sales where id='".$id."'", $dblink));
	$order = array(
		"id" => $id,
		"token_number" => get_token_number( $sale ),
		"datetime_added" => date("h:i A", strtotime($sale[ "datetime_added" ])),
		"ts" => strtotime($sale[ "datetime_added" ]),
		"type" => $sale[ "type" ],
		"waiter_id" => $sale[ "waiter_id" ],
		"dinning_table_id" => $sale[ "dinning_table_id" ],
		...
		"cancel_order_reason" => unslash( $sale[ "cancel_order_reason" ] ),
		"status" => $sale[ "status" ],
		"items" => array(),
		"history" => array()
	);
	if( !empty( $sale[ "customer_payment_id" ] ) ) {
		$customer_payment = doquery( "select * from customer_payment where id = '".$sale[ "customer_payment_id" ]."'", $dblink );
		if( numrows( $customer_payment ) > 0 ) {
			$customer_payment = dofetch( $customer_payment );
			$order[ "account_id" ] = $customer_payment[ "account_id" ];
			$order[ "payment_amount" ] = $customer_payment[ "amount" ];
			$order[ "payment_cleared" ] = true;
		}
	}
	$rs = doquery( "select a.*, b.title, b.unit, b.kitchen_id, c.id as category from sales_items a inner join items b on a.item_id = b.id left join item_category c on b.item_category_id = c.id where sales_id='".$id."' order by c.id, b.sortorder desc", $dblink );
	if( numrows($rs) > 0 ) {
		while( $r = dofetch( $rs ) ) {
	...
	$rs = doquery( "select* from sales_history where sale_id = '".$order[ "id" ]."' order by ts desc", $dblink );
	if( numrows($rs) > 0 ) {
		while( $r = dofetch( $rs ) ) {
			$order["history"][] = array(
				"notes" => $r[ "notes" ]
...
function get_token_number( $order ){
	global $dblink;
	$dt = get_last_closing_date( $order["datetime_added"] );
    $order_id = dofetch( doquery("select count(1) from sales where datetime_added >='".$dt."' and datetime_added<='".$order["datetime_added"]."' and status=1 and id < '".$order[ "id" ]."'", $dblink ) );
...
function get_last_closing_date( $date = "" ){
	...
	return date( "Y-m-d 00:30:00", strtotime( $date ) );
	if( !isset( $GLOBALS["lastclosing"][ $date ] ) ) {
		...
		if( numrows( $rs ) > 0 ) {
			...
			$dt = date( "Y-m-d 00:00:00", strtotime($date) );
		}
		$GLOBALS["lastclosing"][ $date ] = $dt;
	}
	return $GLOBALS["lastclosing"][ $date ];
}
function get_next_closing_date( $date = "" ){
	global $dblink;
	if( empty( $date ) ) {
		$date = date( "Y-m-d" );
	}
	return date( "Y-m-d 00:30:00", strtotime( $date )+3600*24 );
	if( !isset( $GLOBALS["nextclosing"][ $date ] ) ) {
		$rs = doquery( "select * from closing_activity where datetime > '".$date."' order by datetime asc limit 0,1", $dblink );
		if( numrows( $rs ) > 0 ) {
			$r = dofetch( $rs );
			$dt = $r[ "datetime" ];
		}
		else{
			$dt = date( "Y-m-d 23:59:59", strtotime($date)+1 );
		}
		$GLOBALS["nextclosing"][ $date ] = $dt;
	}
	return $GLOBALS["nextclosing"][ $date ];
}

function get_category_name( $cat_id ){
	global $dblink;
 	$cat = dofetch( doquery( "select * from item_category where id='".$cat_id."'", $dblink ) );
	return ($cat[ "parent_id" ] != 0? get_category_name($cat[ "parent_id" ])." &gt; ":"").unslash( $cat[ "title" ] );
}
...
function get_customer_balance( $customer_id, $dt = 0 ){
	...
	if( numrows( $customer ) > 0 ) {
		...
		$sql="select sum(amount) as amount from (select sum(net_price) as amount from sales where customer_id = '".$customer_id."' and datetime_added <'".$dt."' and status=1 union select sum(net_price) as amount from shop_sales where customer_id = '".$customer_id."' and datetime_added <'".$dt."' and status=1 union select -sum(amount) from customer_payment where customer_id = '".$customer_id."' and datetime_added <'".$dt."' and status=1) as transactions";
...
function get_supplier_balance( $supplier_id, $dt = 0 ){
	...
	if( numrows( $supplier ) > 0 ) {
		...
		$sql="select sum(amount) as amount from (select total_price as amount from purchase where supplier_id = '".$supplier_id."' and datetime_added <='".$dt."' and status=1 union select sum(total_price) as amount from assets_purchase where supplier_id = '".$supplier_id."' and datetime_added <'".$dt."' and status=1 union select sum(total_price) as amount from shop_purchase where supplier_id = '".$supplier_id."' and datetime_added <'".$dt."' and status=1 union select -amount from supplier_payment where supplier_id = '".$supplier_id."' and datetime_added <='".$dt."' and status=1) as transactions";
		$balance=dofetch(doquery($sql,$dblink));
...
function get_stock( $item_id, $dt ){
	global $dblink;
	if( empty( $dt ) ) {
		$dt = date( "Y-m-d H:i:s" );
	}
	$item = dofetch( doquery( "select * from shop_items where id = '".$item_id."'", $dblink ) );
	$stock = $item[ "quantity" ];
	$purchase = dofetch( doquery( "SELECT sum(quantity) FROM `shop_purchase_items` a inner join shop_purchase b on a.shop_purchase_id = b.id where b.datetime_added > '".$dt."' and b.status = 1 and a.shop_item_id='".$item_id."'", $dblink ) );
	/*$purchase_return = dofetch( doquery( "SELECT sum(quantity) FROM `purchase_return_items` a inner join purchase_return b on a.purchase_return_id = b.id where b.date > '".$dt."' and b.status = 1 and a.item_id='".$item_id."'", $dblink ) );*/
	$sales = dofetch( doquery( "SELECT sum(quantity) FROM `shop_sales_items` a inner join shop_sales b on a.shop_sales_id = b.id where b.datetime_added > '".$dt."' and b.status = 1 and a.shop_item_id='".$item_id."'", $dblink ) );
	if( $item_id == 29 ) {
		//echo "SELECT sum(quantity) FROM `shop_sales_items` a inner join shop_sales b on a.shop_sales_id = b.id where b.datetime_added > '".$dt."' and b.status = 1 and a.shop_item_id='".$item_id."'";
	}
	/*$sales_return = dofetch( doquery( "SELECT sum(quantity) FROM `sales_return_items` a inner join sales_return b on a.sales_return_id = b.id where b.date > '".$dt."' and b.status = 1 and a.item_id='".$item_id."'", $dblink ) );*/
	$stock = $stock - $purchase[ "sum(quantity)" ] + $sales[ "sum(quantity)" ];// -$sales_return[ "sum(quantity)" ]+ $purchase_return[ "sum(quantity)" ] ;
	return $stock;
}