...
if(isset($_POST["transaction_edit"])){
	...
	if($err==""){
		$sql="Update transaction set `account_id`='".slash($account_id)."',`reference_id`='".slash($reference_id)."',`datetime_added`='".slash(datetime_dbconvert($datetime_added))."',`amount`='".slash($amount)."',`details`='".slash($details)."',`currency_id`='".slash($currency_id)."'"." where id='".$id."'";
		doquery($sql,$dblink);
