...
if(isset($_POST["menu_edit"])){
	...
	if($err==""){
		...
		$sql="Update menu set `title`='".slash($title)."',`url`='".slash($url)."', `parent_id`='".slash($parent_id)."', `small_icon`='".slash($small_icon)."', `depth`='".slash($depth)."', sortorder='".slash( $sortorder )."' where id='".$id."'";
		doquery($sql,$dblink);
