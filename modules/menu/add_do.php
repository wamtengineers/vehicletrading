...
if(isset($_POST["menu_add"])){
	...
	if($err==""){
		...
		$sql="INSERT INTO menu (title, url, parent_id, small_icon, depth, sortorder) VALUES ('".slash($title)."','".slash($url)."','".slash($parent_id)."','".slash($small_icon)."','".slash($depth)."', '".slash( $sortorder )."')";
		doquery($sql,$dblink);
