...
if(isset($_POST["expense_edit"])){
	...
	if($err==""){
		$sql="Update expense set `datetime_added`='".slash(datetime_dbconvert($datetime_added))."',`expense_category_id`='".slash($expense_category_id)."',`account_id`='".slash($account_id)."',`details`='".slash($details)."',`amount`='".slash($amount)."',`currency_id`='".slash($currency_id)."'"." where id='".$id."'";
		doquery($sql,$dblink);
